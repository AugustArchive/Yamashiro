const { readdir, readdirSync, promises: { writeFile } } = require('fs');

module.exports = class YamashiroUtil
{
    /**
    ...
    /**
     * Cleans all AniList HTML code
     * @param {string} html HTML bytecode
     */
    static cleanAniList(html)
    {
        let clean = html
            .replace(/(<br>+)/g, '\n')
            .replace(/^#039;/g, "'")
        ...
        if (spoilers !== 0 && (spoilers && (spilers % 2))) clean += '||';
        return clean;
    }
    ...
    /**
     * Parses a date
     * @param {number} createdAt The timestamp to create
     * @param {"USA" | "Europe" | "Australia" | "Asia"} region The region
     */
    static parseDate(createdAt, region)
    {
        const { dateformat } = require('@yamashiro/modules');

        if (region === 'USA') return dateformat(createdAt, 'mm/dd/yyyy hh:MM:ssTT');
        else return dateformat(createdAt, 'dd/mm/yyyy hh:MM:ssTT');
    }
};
